    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if prec == "amp":
            with autocast():
                ...
                logits_x = torch.einsum('ac,bkc->abk', visual_x, text_u)
                logits_u = torch.einsum('bc,akc->bak', visual_u, text_x)
                logits_x = logits_x.permute(0,2,1)
                logits_x = logits_x*logit_scale
                logits_u = logits_u.permute(0,2,1)
                logits_u = logits_u*logit_scale
                label_new_x = label.expand(self.cfg.DATALOADER.TRAIN_U.BATCH_SIZE,-1).T   #(A,B)
                label_new_y = label_p.expand(self.cfg.DATALOADER.TRAIN_X.BATCH_SIZE,-1).T #(B,A)
                mask_ge_new = mask_ge.expand(self.cfg.DATALOADER.TRAIN_X.BATCH_SIZE,-1).T #(B,A)
