...
#new
class NewContextDecoder(nn.Module):
    def __init__(self,
                 cfg,
    ...
    def forward(self, visual):
        ...
        x = self.in_proj(visual)

        for layer in self.decoder:
            x = layer(x)

        out1 = self.out_proj1(x)
        out2 = self.out_proj2(out1.permute(0,2,1)).squeeze()
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, context_feature = 'attention', use_visual_prompt_generator=True, use_text_prompt_generator=True):
        ...
        #new
        self.new_context_decoder = NewContextDecoder(cfg, transformer_heads=64, transformer_layers=3)
    ...
    def forward(self, img, ind=False, pse=False, fea = False):
        ...
        #new
        new_visual_contexts, new_visual_contexts_pool = self.new_context_decoder(visual_contexts)
        ...
        if fea:
            return_all.append(F.normalize(global_feat.squeeze(),dim=1,p=2))   #(B,1,C)
            return_all.append(F.normalize(updated_vision_embedding,dim=1,p=2)) #(B,C)
        
        #new
        return_all.append(F.normalize(new_visual_contexts_pool,dim=1.p=2))
...
class DAMP(TrainerXU):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if prec == "amp":
            with autocast():
                output_x, output_x_ind, v_x, vprime_x, vtilda_x = self.model(image_x, ind =True, pse=False, fea=True)
                output_u, output_u_ind, pseudo_label_logits, v_u, vprime_u, vtilda_u = self.model(image_u, ind=True, pse=True, fea=True)
                ...
                #new
                loss_new_x = nn.MSELoss(v_x, vtilda_x) + nn.MSELoss(vprime_x, vtilda_x)
                loss_new_u = nn.MSELoss(v_u, vtilda_u) + nn.MSELoss(vprime_u, vtilda_u)
                loss_new = loss_new_x + loss_new_u
