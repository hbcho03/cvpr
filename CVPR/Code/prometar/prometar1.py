...
class ProMetaR(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if len(unique_label) != 1:
            ...
            zero_shot_logits, logits = task_model(x_qry, y_qry, x_qry2, y_qry2, mix_ids, lam=lam, mixup=True) 
            ...
            # new
            if self.batch_idx == 0:
                self.x_qry_save = x_qry
                self.y_qry_save = y_qry
            else:
                self.x_qry_save = torch.cat((self.x_qry_save, x_qry), dim=0)
                self.y_qry_save = torch.cat((self.y_qry_save, y_qry), dim=0)
