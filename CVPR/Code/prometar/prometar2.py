...
def gradient_update(model, loss1, loss2, loss3, loss4, grad_func=None):
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            gradient4 = grad_params4[grad_counter]
            if grad_func:
                if gradient2 == None:
                    gradient = grad_func(gradient1.type(grad_func.dtype), gradient2, gradient3.type(grad_func.dtype), gradient4.type(grad_func.dtype), name)
                elif gradient3 == None:
                    gradient = grad_func(gradient1.type(grad_func.dtype), gradient2.type(grad_func.dtype), gradient3, gradient4.type(grad_func.dtype), name)
                else:
...
class VNet(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.linear_vision_gamma2 = nn.ModuleList([nn.Sequential(nn.Linear(vision_ctx_dim*2, vision_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, bias=False),nn.Linear(vision_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, vision_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, bias=False)).type(self.dtype) for i in range(cfg.TRAINER.PROMETAR.PROMPT_DEPTH_VISION)])
        self.linear_text_gamma2 = nn.ModuleList([nn.Sequential(nn.Linear(text_ctx_dim*2, text_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, bias=False),nn.Linear(text_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, text_ctx_dim//cfg.TRAINER.PROMETAR.DIM_RATE, bias=False)).type(self.dtype) for i in range(cfg.TRAINER.PROMETAR.PROMPT_DEPTH_TEXT)])
        
        
    def forward(self, gradients1, gradients2, gradients3, gradients4, param_name):
        if "image_encoder" in param_name:
            if param_name == "image_encoder.VPT":
                linear_gamma = self.linear_vision_gamma[0]
                linear_gamma2 = self.linear_vision_gamma2[0]
            else:
                ...
                linear_gamma2 = self.linear_vision_gamma2[l_idx]
            ...
            if param_name == "prompt_learner.ctx":
                ...
                linear_gamma2 = self.linear_text_gamma2[0]
            else:
                ...
                linear_gamma2 = self.linear_text_gamma2[l_idx]
        else:
            raise NotImplemented
            
        d_1, d_2 = gradients1.size()
        changed_gradients1, changed_gradients2, changed_gradients3, changed_gradients4 = None, None, None, None 
        if gradients2 == None:
            input_gradients = torch.cat((gradients1, gradients3), 0)
            input_gradients2 = torch.cat((gradients1, gradients4), 0)
            gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1))).repeat_interleave(8,-1).reshape(d_1, d_2)
            gamma_t2 = torch.sigmoid(linear_gamma2(input_gradients2.reshape(1,-1))).repeat_interleave(8,-1).reshape(d_1, d_2)
            changed_gradients1 = gamma_t*gradients3
            changed_gradients2 = gamma_t2*gradients4
            changed_gradients = gradients1 + changed_gradients1 + changed_gradients2
            ...
            gamma_t = torch.sigmoid(linear_gamma(input_gradients.reshape(1,-1))).repeat_interleave(8,-1).reshape(d_1, d_2)
            gamma_t2 = torch.sigmoid(linear_gamma2(input_gradients2.reshape(1,-1))).repeat_interleave(8,-1).reshape(d_1, d_2)
            changed_gradients1 = gamma_t*gradients2
            changed_gradients2 = gamma_t2*gradients4
            changed_gradients = gradients1 + changed_gradients1 + changed_gradients2
        else:
            raise NotImplemented
...
class ProMetaR(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        optim.zero_grad()
        model = gradient_update(model, loss_ce, rag_text, rag_image, rag_logits, grad_func=self.vnet)
        optim.step()
        ...
        if len(unique_label) != 1:
            ...
            # new
            adaptation_rag_logits = F.kl_div(
                F.log_softmax(logits / 1, dim=1),
                F.log_softmax(zero_shot_logits / 1, dim=1),
                reduction='sum',
                log_target=True
            ...
            task_model.adapt(adaptation_loss_ce, adaptation_rag_text, adaptation_rag_image, adaptation_rag_logits, allow_nograd=True, grad_func=self.vnet, allow_unused=True)
